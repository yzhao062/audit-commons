...
The About page's `#contact` section and the site-wide footer provide the editorial contact route in both languages. The current public address is `yzhao062@gmail.com`. Namecheap catch-all email forwarding has been configured to Gmail and MX records are verified, but inbound receipt remains untested; keep the public Gmail address active until receipt has been tested and verified. Update both About bodies, the contribution guide, and the reviewed Chinese source digest together when changing the address.
